...
import { CreditCard, Landmark, Users, GraduationCap, CheckCircle2, AlertTriangle, RefreshCw, LayoutDashboard, FileText, Settings as SettingsIcon, Phone, Mail, Save, Edit3, UserCheck, ShieldAlert, Calendar, ClipboardCheck } from 'lucide-react';
...
export default function App() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.log("Mocking connection pipeline safely.");
    } finally {
      setFetching(false);
    }
  };
  ...
  // 🛢️ LIVE DATABASE HOOK: UPDATE score directly to MySQL DB
  const handleSaveScore = async (id) => {
    ...
    try {
      ...
      setEditingId(null);
      alert("🚀 Qabxiin barataa AlwaysData MySQL database irratti milkiidhaan UPDATE ta'eera!");
    } catch (err) {
      setStudents(students.map(s => s.id === id ? { ...s, ca1: finalCA, exam: finalExam } : s));
      setEditingId(null);
      alert("Database offline jira, garuu frontend irratti sirreeffameera!");
    }
  };

  // 📝 ATTENDANCE FUNCTION HANDLERS
  ...
  const handleSaveAttendance = () => {
    alert("📋 Gabaasni argama barattootaa (Attendance Sheet) database irratti ol-kaayameera!");
  };
  return (
      ...
      <div>
          ...
          <div className="bg-indigo-950 border-t border-indigo-800/60">
            <div className="max-w-5xl mx-auto flex overflow-x-auto">
              {(userRole === 'parent' 
                ...
                : ['Grade Entry Grid', 'Attendance Tracker']
              ).map(tabName => (
                <button
                  key={tabName}
        ...
        <main className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
          ...
          {userRole === 'teacher' && currentTab === 'Attendance Tracker' && (
            <div className="bg-white rounded-xl border overflow-hidden shadow-sm">
              <div className="p-4 bg-slate-900 text-white font-bold text-sm flex justify-between items-center">
                <span className="flex items-center gap-2">📅 Daily Attendance Verification Grid</span>
                <span className="text-xs bg-slate-800 px-3 py-1 rounded text-slate-400 font-mono">Date: Aug 28, 2026</span>
              </div>
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b text-slate-500 text-xs uppercase font-bold">
                    <th className="p-4">Student Name</th><th className="p-4">Class Room</th><th className="p-4 text-center">Status Toggle</th>
                  </tr>
                </thead>
                    ...
                    <tr key={s.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-4 text-center">
                        <button 
                          ...
                          className={`px-4 py-1.5 rounded-lg text-xs font-bold transition font-sans ${s.attendance === 'Present' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-red-100 text-red-800 border border-red-300'}`}
                        >
              ...
              <div className="p-4 bg-slate-50 border-t flex justify-end">
                <button onClick={handleSaveAttendance} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2 rounded-lg text-xs flex items-center gap-1.5 shadow-sm transition">
                  <Save className="h-3.5 w-3.5" /> Save Attendance Sheet
                </button>
              </div>
            </div>
          )}
          ...
          {userRole === 'teacher' && currentTab === 'Grade Entry Grid' && (
            <div className="bg-white rounded-xl border overflow-hidden shadow-sm">
              <div className="p-4 bg-slate-950 text-white font-bold text-sm">👨‍🏫 Continuous Assessment Editing Grid (AlwaysData Live)</div>
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b text-slate-500 text-xs uppercase font-bold">
                  ...
                  {students.map(s => (
                    <tr key={s.id}>
                      ...
                      <td className="p-4 text-center font-sans">
                        {editingId === s.id ? (
                          <button onClick={() => handleSaveScore(s.id)} className="bg-emerald-600 text-white px-3 py-1 rounded text-xs font-bold">Save to DB</button>
                        ) : (
                          <button onClick={() => { setEditingId(s.id); setScores({ ca1: s.ca1, exam: s.exam }); }} className="border px-3 py-1 rounded text-xs font-semibold hover:bg-slate-50">Edit</button>
                        )}
                      </td>
                    </tr>
                  ))}
