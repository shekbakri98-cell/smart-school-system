...
// URL backend Render irratti argamu argachuuf
...
export default function App() {
  ...
  const fetchDashboardData = async (studentId) => {
    ...
    setErrorMessage('');
    try {
      // Maqaa irraa gara ID barataa database tti jijjiiruu
      const idParam = studentId === 'Tariku' ? 'STD-0419' : 'STD-0882';
      
      // Server live Render irra jiru waamuu
      ...
      // State koodii keenyaa data live ta'een update gochuu
      setFinancials({
  ...
  // Barataan yoo jijjiiramu ofumaan data akka fidu gochuu
  useEffect(() => {
  ...
  const handleTelebirrPayment = async () => {
    ...
    try {
      ...
      if (data.code === "0" || data.code === "200") {
        ...
        throw new Error(data.message || "Kaffaltiin lagatameera.");
      }
    } catch (err) {
      ...
      <header className="bg-indigo-900 text-white shadow-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg tracking-tight">HILLSIDE ACADEMY INFOCENTER</h1>
          </div>
                ...
                {transactions.length === 0 ? (
                  ...
                  transactions.map((tx, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-4"><span className="px-2.5 py-1 bg-slate-150 border border-slate-200 rounded text-xs font-semibold text-slate-600 uppercase">{tx.method}</span></td>
                      <td className="p-4 text-right font-black text-slate-900">{tx.amount} <span className="text-xs font-normal text-slate-400">ETB</span></td>
                    </tr>
                  ))
                )}
              </tbody>
