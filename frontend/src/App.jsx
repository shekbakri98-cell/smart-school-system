      ...
      <div>
        ...
        <main className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
          {currentTab !== 'Fees & Payments' ? (
            ...
            <>
              {/* Account Level Selector Row Control */}
              <div className="mb-6 p-4 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setActiveChild('Martha')} className={`px-4 py-1.5 rounded-lg text-xs font-bold border transition ${activeChild === 'Martha' ? 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}${activeChild === 'Martha' ? 'bg-indigo-900 border-indigo-900 text-white shadow-sm' : ''}`}>Martha Abebe (Grade 4A)</button>
                  </div>
                </div>
              ...
              {/* Invoices Ledger Overview Section */}
              ...
              {/* Transactions Table Log Layout Component */}
              ...
              {/* PDF Action Downloader Module */}
Download Full Statement as PDF
            </>
    )}
          {/* Dynamic Static Support Footer Layout */}
          Support Helpline: +251-11-XXXXXXX 
          Emergency Email Support: support@school.edu.et
          );
          }
